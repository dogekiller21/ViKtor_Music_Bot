...
class MusicCog(commands.Cog, name="Music"):
    ...
    def _after(self, error: Exception, guild: Guild):
        ...
        guild.voice_client.play(
            source=get_source(
                ...
                volume_level=db_settings.volume_option / 100,
            ),
    ...
    async def leave_channel(self, ctx: ApplicationContext):
        ...
        await ctx.respond(embed=BotEmbeds.info_embed(description="Bye!"))
    ...
    async def lyrics_command(self, ctx: ApplicationContext):
        current_track = self.storage.get_queue(
        ...
        await ctx.defer()
        lyrics = await self.lyrics_parser.search(
            q=current_track.get_full_name(max_length=100)
        )
        embed = BotEmbeds.info_embed(
    ...
    async def ensure_self_voice_and_join(self, ctx: ApplicationContext):
        await ctx.defer()
        try:
            await asyncio.wait_for(join_author_voice(ctx=ctx), timeout=15)
    ...
    async def on_error_play_single_track(
        ...
        if isinstance(error, CheckFailure):
            ...
            return
        if isinstance(error, SingleTrackParsingApiError):
            await ctx.respond(
                embed=BotEmbeds.error_embed(
    ...
    async def on_error_play_playlist(
        ...
        if isinstance(error, IncorrectPlaylistUrlException):
            ...
            return
        if isinstance(error, PlaylistParsingApiError):
            await ctx.respond(
                embed=BotEmbeds.error_embed(
                    description="Error while parsing playlist, doublecheck the link and try again"
                ),
                ephemeral=True,
            )
            return
    ...
    async def on_lyrics_error(
        ...
        if isinstance(error, SongNotFoundException):
            return await ctx.respond(
                embed=BotEmbeds.error_embed(
                    description="Lyrics not found for this song :("
                ),
                ephemeral=True,
            )
        if isinstance(error, LyricsParsingError):
            return await ctx.respond(
                embed=BotEmbeds.error_embed(
                    description="Error occurred while getting lyrics"
                ),
                ephemeral=True,
            )
