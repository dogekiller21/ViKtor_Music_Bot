...
@update_json(PlayListsFile)
def rename_playlist(guild_id: int, old_name: str, new_name: str, json_data: JSON_DATA) -> JSON_DATA:
...
@update_json(PlayListsFile)
def delete_playlist(guild_id: int, playlist_name: str, json_data: JSON_DATA) -> JSON_DATA:
    guild_playlists = get_single_guild_playlist(guild_id)
    if guild_playlists is None:
        raise NoGuildPlaylists
