...
class LyricsParser:
    ...
    async def _parse_lyrics(self, lyrics_path: str):
        soup = BeautifulSoup(await self.make_request(
            url=f"{BASE_URL}{lyrics_path}",
            return_json=False
        ), "lxml")
        return soup.find("div", attrs={"data-lyrics-container": True}).get_text(separator="\n\n", strip=True)
    ...
    async def search(self, q: str) -> Optional[str]:
        ...
        if json_data is None:
            return
        songs = [
            hit["result"] for hit in json_data["response"]["hits"]
            if hit["type"] == "song"
        ]
        if not songs:
