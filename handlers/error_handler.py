...
async def command_error_handler(ctx: ApplicationContext, error: DiscordException):
    if isinstance(error, ApplicationCommandInvokeError) and isinstance(
        error.original, UserVoiceException
    ):
        embed = Embeds.error_embed(
            description="You are not connected to the voice channel"
        )
    ...
    elif isinstance(error, ApplicationCommandInvokeError) and isinstance(
        error.original, EmptyQueueException
    ):
        embed = Embeds.error_embed(description="Queue is empty. Nothing is playing")
        await ctx.respond(embed=embed, ephemeral=True)
    elif isinstance(error, ApplicationCommandInvokeError) and isinstance(
        error.original, IncorrectLinkException
    ):
